...
parser.add_argument('-t', '--train-batchsize', default=20, type=int, metavar='T',
                    help='numbers of train batches per epoch')
parser.add_argument('-v', '--validation-batchsize', default=20, type=int, metavar='T',
                    help='numbers of valitaion batches')
        ...
        if i % args.print_freq == 0:
            print('Epoch: [{0}][{1}/{2}]\t'
                  ...
                  'Prec@5 {top5.val:.3f} ({top5.avg:.3f})'.format(
                   epoch, i, args.train_batchsize, batch_time=batch_time,
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        if i % args.print_freq == 0:
            print('Test: [{0}/{1}]\t'
                  ...
                  'Prec@1 {top1.val:.3f} ({top1.avg:.3f})\t'
                  'Prec@5 {top5.val:.3f} ({top5.avg:.3f})'.format(
                   i, args.validation_batchsize, batch_time=batch_time, loss=losses,
                   top1=top1, top5=top5))
        if i == args.validation_batchsize:
            break
